...
class MultiXScienceDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        if self.aggregation:
            data = data.to_pandas().groupby("aid").agg({
...
class PromptMultiXScienceDataModule(MultiXScienceDataModule):
    ...
    def __init__(
        ...
        super().__init__(model_config, path, save_path, doc_sep,
                         aggregation, num_workers, max_length, num_proc_tokenization)
        self.prompt = prompt
...
class RGMultiXScienceDataModule(MultiXScienceDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        for k in data.keys():
            ...
            data_dict.update({k: datasets.Dataset.from_pandas(
                v, split=k, preserve_index=True)})
        ...
        def tokenize(x) -> dict:
            query = query_tokenizer(
                text=x['query_input'], max_length=self.query_max_length, **tokenizer_kwargs)
    ...
    def _get_dataloader(self, mode: str, batch_size: int) -> DataLoader:
        dataset = self.data[mode].select_columns(
            column_names=['input_ids', 'attention_mask', 'query_input_ids', 'query_attention_mask',
                          'labels', 'input', 'query_input', 'target', 'aid', 'aid_counts', 'index'],
        )
        dataset.set_format(
            type="torch",
            columns=['input_ids', 'attention_mask',
                     'query_input_ids', 'query_attention_mask', 'labels'],
            output_all_columns=True,
        )
